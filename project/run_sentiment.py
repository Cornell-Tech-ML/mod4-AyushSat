...
class CNNSentimentKim(minitorch.Module):
    ...
    def __init__(
        ...
        self.conv2 = Conv1d(embedding_size, feature_map_size, filter_sizes[0])
    ...
    def forward(self, embeddings):
        ...
        res1 = self.conv1(embeddings).relu()
        res2 = self.conv2(embeddings).relu()
        res3 = self.conv3(embeddings).relu()

        maxp1 = minitorch.max(res1, dim=2)
        maxp2 = minitorch.max(res2, dim=2)
        maxp3 = minitorch.max(res3, dim=2)

        comb = (maxp1 + maxp2 + maxp3).view(embeddings.shape[0], self.feature_map_size)
        lin = self.linear(comb)
        dropout = minitorch.dropout(lin, self.dropout, not self.training)
